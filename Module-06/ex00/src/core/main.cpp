...
int check_input(const char *input)
{
        ...
        if (!str.compare("nan") || !str.compare("nanf") || !str.compare("inf") || !str.compare("inff") || !str.compare("-inf") || !str.compare("-inff") || !str.compare("+inf") || !str.compare("+inff"))
                return 1;
        ...
        for (int i = 0; input[i]; i++)
        {
                if (input[i] == '.')
                        dot++;
                if (dot > 1 || !is_charset(input[i], "+-.0123456789f") || (is_charset(input[i], "+-") && i != 0) || (input[i] == 'f' && input[i + 1]))
                        return input_error();
        }
        return 1;
}
