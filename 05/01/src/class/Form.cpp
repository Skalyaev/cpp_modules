...
Form::Form() : name("UNDEFINED"), sGrade(150), eGrade(150), isSigned(0), isExecuted(0)
{
        std::cout << "[ DEFAULT ][ " << this << " ] Form constructor called" << std::endl;
}

Form::Form(const Form &src) : name(src.getName()), sGrade(src.getSgrade()), eGrade(src.getEgrade()), isSigned(src.checkSignature()), isExecuted(src.checkExec())
{
        std::cout << "[ COPY ][ " << this << " ] Form constructor called" << std::endl;
}

Form::Form(const std::string &str, const short &sNbr, const short &eNbr) : name(str), isSigned(0), isExecuted(0)
{
