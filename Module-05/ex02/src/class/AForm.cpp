#include "../../include/class/AForm.hpp"

AForm::AForm() : name("UNDEFINED"), target("UNDEFINED"), sGrade(150), eGrade(150), isSigned(0), isExecuted(0)
{
        std::cout << "[ DEFAULT ][ " << this << " ] AForm constructor called" << std::endl;
}

AForm::AForm(const AForm &src) : name(src.getName()), target(src.getTarget()), sGrade(src.getSgrade()), eGrade(src.getEgrade()), isSigned(src.checkSignature()), isExecuted(src.checkExec())
{
        std::cout << "[ COPY ][ " << this << " ] AForm constructor called" << std::endl;
}

AForm::AForm(const std::string &nStr, const std::string &tStr, const short &sNbr, const short &eNbr) : name(nStr), target(tStr), isSigned(0), isExecuted(0)
{
